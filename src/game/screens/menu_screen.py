...
class MenuScreen(BaseScreen):
    ...
    def handle_event(self, event):
        ...
        if event.type == pygame.KEYDOWN and event.key == pygame.K_b:
            self.game.start_boss_test("leaping")
    ...
    def draw(self, surface):
        ...
        new_boss = self.info_font.render("Pulsa B para probar el jefe saltador", True, config.HUD_COLOR)
        ...
        surface.blit(new_boss, new_boss.get_rect(center=(surface_width // 2, surface_height // 2 + 30)))
        surface.blit(fullscreen, fullscreen.get_rect(center=(surface_width // 2, surface_height // 2 + 40)))
